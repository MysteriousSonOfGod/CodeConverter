...
SmoothedMovingAverage::SmoothedMovingAverage(const std::string& symbol, const int timeframe, const int period,
	const int price_type) :
	MovingAverage{ symbol, timeframe, period, price_type }
{
}

SmoothedMovingAverage::SmoothedMovingAverage(const std::vector<double>* const history_vector, const int period) :
...
void SmoothedMovingAverage::update()
{
	...
	for (int i = m_last_calculated; i < bars; ++i)
	{
		...
		{
			m_values[i] = (m_values[i - 1] * (m_period - 1) + curr_price) / m_period;
		}
...
double SmoothedMovingAverage::getValue(const int index) const
{
	const int bars = m_history.Bars();
