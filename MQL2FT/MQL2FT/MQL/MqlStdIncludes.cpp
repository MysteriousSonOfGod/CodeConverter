...
// Helper functions
string charArrToStr(Array<char>& arr)
{
	string tmp;
...
void stringToCharArr(const string& str, Array<char>& arr)
{
	string tmp;
...
string DoubleToStrMorePrecision(double number, int precision)
{
	...
	integer = mql4::MathFloor(number);
	rem = mql4::MathRound((number-integer)*p);
	remstring = "";
	for(int i = 0; i<precision; i++)
	{
		integer2 = mql4::MathFloor(rem/10);
		rem2 = (int)NormalizeDouble(rem-integer2*10, 0);
		remstring = IntegerToString(rem2)+mql4::string(remstring);
		rem = integer2;
	}
	//---
...
{
	return SendNotifyMessageA((HWND)hWnd, Msg, wParam, lParam);
}

int SendNotifyMessageW(int hWnd, int Msg, int wParam, int lParam)
{
	return SendNotifyMessageW((HWND)hWnd, Msg, wParam, lParam);
}
...
{
	return PostMessageW((HWND)hWnd, Msg, wParam, lParam);
}

//---- windows
int SetWindowTextA(int hWnd, Array<char>& lpString)
{
	string str = charArrToStr(lpString);
	return SetWindowTextA((HWND)hWnd, str.c_str());
}

int SetWindowTextW(int hWnd, string lpString)
{
	// TODO: Implement correct convertion to wchar, if needed.
	return SetWindowTextW((HWND)hWnd, (const wchar_t*)lpString.c_str());
}

int GetWindowTextA(int hWnd, Array<char>& lpString, int nMaxCount)
{
	int result;
	char* buf = new char[nMaxCount];

	result = GetWindowTextA((HWND)hWnd, buf, nMaxCount);
	string str(buf);
	delete[] buf;

	stringToCharArr(str, lpString);
...
{
	int result;
	wchar_t* buf = new wchar_t[nMaxCount];
	
	result = GetWindowTextW((HWND)hWnd, buf, nMaxCount);

	// This function cannot possibly return usefull information, useless!
...
{
	return GetWindowTextLengthA((HWND)hWnd);
}
...
{
	return (int)GetWindow((HWND)hWnd, uCmd);
}
...
{
	return (int)SetActiveWindow((HWND)hWnd);
}

int AnimateWindow(int hWnd, int dwTime, int dwFlags)
{
	return AnimateWindow((HWND)hWnd, dwTime, dwFlags);
}

int FlashWindow(int hWnd, int dwFlags /*bInvert*/)
{
	return FlashWindow((HWND)hWnd, dwFlags);
}
...
{
	string text = charArrToStr(lpText);
	string cap = charArrToStr(lpCaption);

	return MessageBoxA((HWND)hWnd, text.c_str(), cap.c_str(), uType);
}

int MessageBoxW(int hWnd, string lpText, string lpCaption, int uType)
{
	// TODO: Implement correct convertion to wchar, if needed.
	return MessageBoxW((HWND)hWnd, (const wchar_t*)lpText.c_str(), (const wchar_t*)lpCaption.c_str(), uType);
}
...
int MessageBoxExA(int hWnd, Array<char>& lpText, Array<char>& lpCaption, int uType, int wLanguageId)
{
	...
	return MessageBoxExA((HWND)hWnd, text.c_str(), cap.c_str(), uType, wLanguageId);
}

int MessageBoxExW(int hWnd, string lpText, string lpCaption, int uType, int wLanguageId)
{
	// TODO: Implement correct convertion to wchar, if needed.
	return MessageBoxExW((HWND)hWnd, (const wchar_t*)lpText.c_str(), (const wchar_t*)lpCaption.c_str(), uType, wLanguageId);
}